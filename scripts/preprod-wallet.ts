...
async function registerForDust() {
  ...
  const environment = getMidnightEnvironment({
    ...
    MIDNIGHT_PROOF_SERVER_URL: `http://127.0.0.1:${process.env.COVENANT_PROOF_PORT ?? '16300'}`,
  });
  ...
  try {
    ...
    console.log('Synchronizing the Preprod wallet. This can take several minutes...');
    const state = await keys.wallet.waitForSyncedState();
